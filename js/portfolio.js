$(document).ready(function(){
				...
				$('.portfolio_content').find('span.portfolio_info').hide();
		$('.portfolio_content').hoverIntent(
		  function () {
			$(this).find('span.portfolio_info').stop().slideDown();
		  },
		  function () {
			$(this).find('span.portfolio_info').stop().slideUp();
		  }
		);
		...
		//preload the images				
		$('.portfolio_container:first').find('img').preload({
			onComplete	: function(){
				$('.fp_loading_main').css({'display':'none'});
				$('#fp_galleryList>li:first').trigger('click');
			}
		});
				...
				function openGallery($gallery){
					...
					var $fp_content_wrapper = $fp_thumbContainer.find('.portfolio_container:nth-child('+parseInt(gallery_idx+1)+')');
					//hide all the other galleries thumbs wrappers
					$fp_thumbContainer.find('.portfolio_container').not($fp_content_wrapper).hide();
					//and show this one
					...
					*/
					...
					$fp_thumbContainer.animate({'height':'435px'},200,function(){
					...
					$fp_content_wrapper.find('.portfolio_content')
				...
				$fp_close.bind('click',function(){
					...
					//the current thumb
					var $current 		= $fp_thumbScroller.find('.portfolio_container:nth-child('+parseInt(gallery_idx+1)+')')
														   .find('.portfolio_content:nth-child('+parseInt(current+1)+')');
				...
				function slideThumb(way){
					if(way==1){
						++current;
						var $next = $fp_thumbScroller.find('.portfolio_container:nth-child('+parseInt(gallery_idx+1)+')')
													 .find('.portfolio_content:nth-child('+parseInt(current+1)+')');
						if($next.length > 0)
						...
						--current;
						var $prev = $fp_thumbScroller.find('.portfolio_container:nth-child('+parseInt(gallery_idx+1)+')')
													 .find('.portfolio_content:nth-child('+parseInt(current+1)+')');
						if($prev.length > 0)
				...
				function checkClosest(){
					...
					var idx					= 0;	
					$container				= $fp_thumbScroller.find('.portfolio_container:nth-child('+parseInt(gallery_idx+1)+')');
					$container.find('.portfolio_content').each(function(i){
						var $obj 		= $(this);
						//the offset left of the element
						var obj_left 	= $obj.offset().left;
						//the center of the element is its offset left plus 
					...
					var $new_current 	= $container.find('.portfolio_content:nth-child('+parseInt(idx+1)+')');
